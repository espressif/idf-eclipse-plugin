...
Next, we need to tell CDT to use the toolchain for our project so that all the headers will be indexed and resolved. This is accomplished through the Launch Bar, the new widget set you see on the far left of the toolbar. This will be shown only when you have a project in the project explorer.  If you don't have the launch bar, you need to install it with the complete C/C++ IDE from the eclipse marketplace.
